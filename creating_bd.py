...
async def calculate_30_procents(id_tg):
    ...
    arkans = arkans * 0.3
    arkans = int(arkans)
    minuses = await db.execute(
        """SELECT minuses FROM users WHERE id_tg = ?""", (id_tg,)
    )
...
async def get_users_to_progrev():
    ...
    db = await aiosqlite.connect(db_path)
    total_users = await db.execute(f'''SELECT id_tg, num_progrev FROM users 
                                   WHERE time_register >= "{str_start}" and time_register <= "{str_end}" and
                                   status < 9 and num_progrev < 3"''')
